...
- A desktop in-app Chromium playtest reached results in all six playable modes, including narrow viewport checks at 390px and 320px. A fixed-height report box clipped wrapped lines; reports now grow to show their full text and reflow on resize. See the [issue #12 browser evidence and device checklist](docs/issue-12-playtest.md). Narrow desktop emulation is not real iPhone/iPad Safari testing.
- Real iPhone/iPad Safari touch, rotation, app switching, clipboard fallback, and balance checks remain pending. Mobile ChatGPT Work connected-browser sessions have not reached the local Vite address; record real-device results separately before treating issue #12 as complete.

Asteroid Field / Pilot, Gunner, Bomber, and Life Support plus Space Battle / Pilot and Bomber are implemented on `main`. Shared setup, controls, and manual reporting are present; issues #2, #3, and #12 remain open for their remaining acceptance evidence.

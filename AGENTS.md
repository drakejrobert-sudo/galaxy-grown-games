...
Keep simulation and balance in src/game/rules.ts, rendering in src/game/scene.ts, input in src/game/input.ts, and the UI shell in src/main.ts. Use the existing stack unless a change is requested. Bomber and Gunner never steer the ship: flight is automatic. Bomber uses a separate weapon reticle and independent mine/missile actions; keyboard aiming is normalized and speed-capped, while touch/mouse place targets directly with the same weapon cooldowns. Preserve Pilot steering. Keep unsupported roles visibly disabled rather than implying they work.
